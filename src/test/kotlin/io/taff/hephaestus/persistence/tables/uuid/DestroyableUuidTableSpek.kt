...
data class DestroyableUuidRecord(override var title: String? = null,
    ...
    : TitleAware, Model<UUID>, DestroyableModel<UUID>
...
object DestroyableUuidTableSpek  : Spek({
    ...
    includeDestroyableTableSpeks(destroyableUuidTable,
        recordFxn = { DestroyableUuidRecord(title = "Soul Food") },
        directUpdate = { record, newTitle, scope ->
            when(scope) {
                DestroyableScope.LIVE -> destroyableUuidTable
                DestroyableScope.DELETED -> destroyableUuidTable.destroyed()
                DestroyableScope.ALL -> destroyableUuidTable.liveAndDestroyed()
            }.update({ destroyableUuidTable.id eq record.id }) { it[titleColumn!!] = newTitle }
        })
})
